...
def login_system():
    # Restaurar sessão a partir de query params (mantém usuário logado após reload)
    try:
        qp = st.query_params
    except Exception:
        try:
    ...
    if qp and 'user' in qp:
        user_val = qp.get('user')
        if isinstance(user_val, (list, tuple)):
            user_val = user_val[0] if user_val else ''
        if user_val and not st.session_state.get('logged_in', False):
            st.session_state['logged_in'] = True
            st.session_state['username'] = user_val

    if st.session_state.get("logged_in", False):
        with st.sidebar:
            st.markdown(f"<div class='section-title' style='color: #ffffff;'>👤 <span style='color:#ffffff'>Logado como:</span> <strong style='color:#ffffff'>{st.session_state.get('username','')}</strong></div>", unsafe_allow_html=True)
        return True

    cols = st.columns([1, 2, 1])
    placeholder = cols[1].empty()
    with placeholder.container():
        st.markdown("<div style='max-width:760px;margin:0 auto;padding:20px;'>", unsafe_allow_html=True)
        st.markdown("<h2 style='margin-bottom:6px;'>🔒 Acesso ao Gerador de Horários</h2>", unsafe_allow_html=True)
        st.markdown("<div class='small-muted'>Faça login para acessar as funções de geração de horários.</div>", unsafe_allow_html=True)

        logo_col1, logo_col2 = st.columns([1, 3])
        with logo_col1:
            st.markdown("<div class='login-card' style='background:linear-gradient(180deg,#0b69a3,#0b5f88);color:white;padding:10px;border-radius:8px;text-align:center;font-weight:700;'>ESCOLA</div>", unsafe_allow_html=True)
        with logo_col2:
            st.markdown("<div style='font-size:14px;margin-top:6px;'>Bem-vindo — insira suas credenciais abaixo.</div>", unsafe_allow_html=True)

        with st.form(key='login_form'):
            uname = st.text_input('Usuário', value=st.session_state.get('username',''))
            pwd = st.text_input('Senha', type='password')
            submitted = st.form_submit_button('Entrar')

            if submitted:
                try:
                    users_db = st.secrets.get('users', {'admin':'admin'})
                except Exception:
                    users_db = {'admin':'admin'}

                if uname and uname in users_db and users_db[uname] == pwd:
                    st.session_state['logged_in'] = True
                    ...
                    try:
                        if hasattr(st, 'experimental_rerun'):
                            # também salvar nome do usuário nos query params para persistência
                            try:
        ...
        st.markdown("<div style='margin-top:8px' class='help-box'><strong>Ajuda:</strong> Se você não tiver credenciais, solicite acesso ao administrador.</div>", unsafe_allow_html=True)

        # botão para solicitar acesso via WhatsApp com mensagem pré-definida
        phone = "+5567999455111"
        message = "Ol%C3%A1%2C%20gostaria%20de%20solicitar%20acesso%20ao%20Gerador%20de%20Hor%C3%A1rios.%20Meu%20nome%3A%20%5BSEU%20NOME%5D%20-%20Escola%3A%20%5BNOME%20DA%20ESCOLA%5D"
        wa_url = f"https://api.whatsapp.com/send?phone={phone}&text={message}"
        st.markdown(f"<div style='margin-top:10px;text-align:center;'><a href='{wa_url}' target='_blank' style='text-decoration:none;'><button style='background:linear-gradient(90deg,#0b69a3,#1e90ff);color:white;border:none;padding:10px 16px;border-radius:6px;font-weight:600;'>📩 Solicitar Acesso via WhatsApp</button></a></div>", unsafe_allow_html=True)
